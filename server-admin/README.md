...
- エンドポイント:
  - `GET  /status` … サービス稼働/自動起動状況＋システム情報（uptime/load/disk/mem）
  - `POST /service` … `{ "service": "<id>", "action": "start|stop|restart|enable|disable" }`
  - `POST /deploy` … `git pull --ff-only`（REPO_DIR）。差分ファイルとHEADを返す
  - `POST /nginx/reload` … `nginx -t` 後に問題なければ `systemctl reload nginx`
  - `POST /reboot` … サーバー再起動（`systemctl reboot`）
  - `GET  /logs?service=<id>&lines=100` … journalctl ログ
  - `GET  /comments?slug=<slug>` / `POST /comments/delete {id}` … CEGコメントの閲覧・削除
  - `GET  /health`
- 環境変数（任意）: `REPO_DIR`（既定 `/home/shodai/tokusho`）, `COMMENTS_DB`
  （既定 `/home/shodai/data/ceg_comments.db`／comments-api と同じDBを直接参照）
- `tokusho-admin`（自身）の stop / disable はロックアウト防止のためブロック。
- `/deploy` は systemd 経由（SSHエージェント無し）で git pull するため、GitHub への鍵は
  **パスフレーズ無し**かつ `~/.ssh/known_hosts` に github.com が登録済みである必要がある。
